...
async function catalog(version) {
  ...
  app.innerHTML = `${home ? hero() : '<div class="breadcrumb"><a href="/">หน้าแรก</a> / สินค้าทั้งหมด</div>'}<section aria-labelledby="catalog-title"><div class="section-head"><div><div class="eyebrow">อุปกรณ์การแพทย์ให้เช่า</div><h${home ? "2" : "1"} id="catalog-title">${home ? "เลือกอุปกรณ์สำหรับการดูแลที่บ้าน" : "สินค้าทั้งหมด"}</h${home ? "2" : "1"}><p>ดูรายละเอียดรุ่น ระยะเวลาเช่า และเงินมัดจำ</p></div>${home ? '<a class="small-link" href="/catalog">ดูสินค้าทั้งหมด ↗</a>' : ""}</div><label class="category-filter" for="category-filter"><span>หมวดหมู่สินค้า</span><select id="category-filter"><option value="">ทุกหมวดหมู่ (${categories.reduce((n, c) => n + Number(c.count), 0)})</option>${categories.map((c) => `<option value="${esc(c.id)}" ${selected === c.id ? "selected" : ""}>${esc(c.name)} (${c.count})</option>`).join("")}</select></label><div class="toolbar"><form class="search" id="search-form" role="search"><span aria-hidden="true">⌕</span><input name="q" aria-label="ค้นหาสินค้า" placeholder="ค้นหาสินค้า ชื่อ หรือรหัสสินค้า…" value="${esc(params.get("q") || "")}"></form><label class="available"><input type="checkbox" id="available" ${params.get("available") === "true" ? "checked" : ""}>เฉพาะสินค้าพร้อมจำหน่าย</label><select id="sort" aria-label="เรียงลำดับสินค้า">${[
